...
const Sidebar = ({ brand = "Smart Kitchen", links, open, onClose }) => {
  ...
  return (
    <>
      <aside
        className={`fixed top-0 left-0 h-full bg-slate-950 border-r border-slate-800 text-slate-100 shadow-lg z-40 transform transition-transform duration-300 w-64 lg:translate-x-0 ${
            ...
            {links.map((link) => {
              ...
              return (
                <li key={link.path}>
                  <NavLink
                    to={link.path}
                    className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm hover:bg-slate-800 transition-all relative ${
                      active
                        ? "bg-slate-800 font-semibold before:absolute before:left-0 before:top-0 before:h-full before:w-1 before:bg-emerald-400"
                        : "text-slate-300"
                    }`}
                    onClick={onClose}
                  >
